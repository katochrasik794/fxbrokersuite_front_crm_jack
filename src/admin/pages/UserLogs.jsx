...
export default function UserLogs() {
  ...
  useEffect(() => {
    ...
    fetch(`${BASE}/admin/logs/user?${params.toString()}`, {
  ...
  const columns = useMemo(() => [
    ...
    { key: "created_at", label: "Timestamp", render: (v) => fmtDate(v) },
    {
      ...
      render: (_, row) => (
        <button
          onClick={() => navigate(`/admin/logs/user/detail/${row.id}`)}
  ...
  const handleExport = () => {
    const csv = [
      ['User Email', 'Action Type', 'Category', 'Target', 'Target ID', 'Description', 'IP Address', 'Timestamp'].join(','),
      ...rows.map(r => [
        r.user_email || '',
        r.action_type || '',
        r.action_category || '',
