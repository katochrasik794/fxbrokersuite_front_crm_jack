...
// Admin > Reports > Bonus Deposit Report
export default function ReportsBonusDeposits() {
  ...
  const BASE = import.meta.env.VITE_BACKEND_API_URL
    ...
    || "https://fxbrokersuite-back-crm-jack.onrender.com/api";
  ...
  useEffect(() => {
    ...
    // Pull directly from consolidated admin transactions (bonus_add)
    fetch(`${BASE}/admin/admin-transactions?operation_type=bonus_add&limit=500`, {
  ...
  const columns = useMemo(() => ([
    ...
    { key: "amount", label: "Bonus Amount", render: v => `$${Number(v||0).toFixed(2)}` },
  ...
  const filters = useMemo(() => ({
    searchKeys: ["userEmail","userName","mt5AccountId","bonusType","status","note"],
    dateKey: "createdAt",
  }), []);

  if (loading) return <div className="rounded-xl bg-white border border-gray-200 p-4">Loading bonus deposits…</div>;
  if (error) return <div className="rounded-xl bg-white border border-rose-200 text-rose-700 p-4">{error}</div>;

  return (
    <ProTable
      title="Bonus Deposit Report"
      rows={rows}
      columns={columns}
      filters={filters}
      pageSize={10}
